...
do_ndk_build () {
    ...
    case "$PARAM_TARGET" in
        armv5|armv7a)
            cd "probe/ffprobe-$PARAM_TARGET/src/main/jni"
...
case "$REQUEST_TARGET" in
    ...
    *)
        echo "Usage:"
        echo "  compile-ffprobe.sh armv5|armv7a|arm64|x86|x86_64"
        echo "  compile-ffprobe.sh all|all32"
        echo "  compile-ffprobe.sh all64"
        echo "  compile-ffprobe.sh clean"
    ;;
esac
