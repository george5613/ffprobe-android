...
static int writer_open(WriterContext **wctx, const Writer *writer, const char *args,
    ...
    if (args) {
        ...
        if ((ret = av_dict_parse_string(&opts, args, "=", ":", 0)) < 0) {
            LOGE("Failed to parse option string '%s' provided to writer context\n", args);
            av_dict_free(&opts);
            goto fail;
        }
        ...
        while ((opt = av_dict_get(opts, "", opt, AV_DICT_IGNORE_SUFFIX))) {
            if ((ret = av_opt_set(*wctx, opt->key, opt->value, AV_OPT_SEARCH_CHILDREN)) < 0) {
                LOGE("Failed to set option '%s' with value '%s' provided to writer context\n",
                       opt->key, opt->value);
                av_dict_free(&opts);
                goto fail;
            }
...
#define PRINT_LAST(b, size, f, ...) do {        \
    int offset = strlen(b);                    \
    if(offset < size) {                        \
        snprintf(&(b[offset]), size - offset, f, ##__VA_ARGS__); \
    }                                          \
} while(0)
...
static void default_print_section_header(WriterContext *wctx) {
    ...
    if (!(section->flags & (SECTION_FLAG_IS_WRAPPER | SECTION_FLAG_IS_ARRAY)))
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "[%s]\n", upcase_string(buf, sizeof(buf), section->name));
}
...
static void default_print_str(WriterContext *wctx, const char *key, const char *value) {
    ...
    if (!def->nokey)
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s%s=", wctx->section_pbuf[wctx->level].str, key);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s\n", value);
}
...
static void default_print_int(WriterContext *wctx, const char *key, long long int value) {
    ...
    if (!def->nokey)
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s%s=", wctx->section_pbuf[wctx->level].str, key);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%lld\n", value);
}
...
static av_cold int compact_init(WriterContext *wctx) {
    ...
    if (strlen(compact->item_sep_str) != 1) {
        LOGE("Item separator '%s' specified, but must contain a single character\n",
...
static void compact_print_section_header(WriterContext *wctx) {
    ...
    if (!(section->flags & SECTION_FLAG_IS_ARRAY) && parent_section &&
        ...
        if (parent_section && compact->has_nested_elems[wctx->level - 1] &&
            ...
            PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "\n");
        }
        if (compact->print_section &&
            !(section->flags & (SECTION_FLAG_IS_WRAPPER | SECTION_FLAG_IS_ARRAY)))
            PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s%c", section->name, compact->item_sep);
    }
}
...
static void compact_print_str(WriterContext *wctx, const char *key, const char *value) {
    ...
    if (wctx->nb_item[wctx->level]) PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%c", compact->item_sep);
    if (!compact->nokey)
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s%s=", wctx->section_pbuf[wctx->level].str, key);
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s", compact->escape_str(&buf, value, compact->item_sep, wctx));
...
static void compact_print_int(WriterContext *wctx, const char *key, long long int value) {
    ...
    if (!compact->nokey)
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s%s=", wctx->section_pbuf[wctx->level].str, key);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%lld", value);
}
...
static void flat_print_int(WriterContext *wctx, const char *key, long long int value) {
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s%s=%lld\n", wctx->section_pbuf[wctx->level].str, key, value);
}
...
static void flat_print_str(WriterContext *wctx, const char *key, const char *value) {
    ...
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s", wctx->section_pbuf[wctx->level].str);
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s=", flat_escape_key_str(&buf, key, flat->sep));
    av_bprint_clear(&buf);
...
static void ini_print_section_header(WriterContext *wctx) {
    ...
    av_bprint_clear(buf);
    if (!parent_section) {
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "# ffprobe output\n\n");
        return;
    }
...
static void ini_print_str(WriterContext *wctx, const char *key, const char *value) {
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s=", ini_escape_str(&buf, key));
    av_bprint_clear(&buf);
...
static void ini_print_int(WriterContext *wctx, const char *key, long long int value) {
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s=%lld\n", key, value);
}
...
#define JSON_INDENT() PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%*c", json->indent_level * 4, ' ')
...
static void json_print_section_header(WriterContext *wctx) {
    ...
    if (section->flags & SECTION_FLAG_IS_WRAPPER) {
        ...
        if (section->flags & SECTION_FLAG_IS_ARRAY) {
            ...
            PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "\"%s\": {%s", buf.str, json->item_start_end);
        } else {
            PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "{%s", json->item_start_end);

            /* this is required so the parser can distinguish between packets and frames */
            if (parent_section && parent_section->id == SECTION_ID_PACKETS_AND_FRAMES) {
                if (!json->compact)
                    JSON_INDENT();
                PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "\"type\": \"%s\"%s", section->name, json->item_sep);
            }
        }
        av_bprint_finalize(&buf, NULL);
    }
...
static inline void json_print_item_str(WriterContext *wctx,
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "\"%s\":", json_escape_str(&buf, key, wctx));
    av_bprint_clear(&buf);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, " \"%s\"", json_escape_str(&buf, value, wctx));
    av_bprint_finalize(&buf, NULL);
}
...
static void json_print_str(WriterContext *wctx, const char *key, const char *value) {
    ...
    if (wctx->nb_item[wctx->level])
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s", json->item_sep);
    if (!json->compact)
        JSON_INDENT();
...
static void json_print_int(WriterContext *wctx, const char *key, long long int value) {
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "\"%s\": %lld", json_escape_str(&buf, key, wctx), value);
    av_bprint_finalize(&buf, NULL);
}
...
#define XML_INDENT() PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%*c", xml->indent_level * 4, ' ')
...
static void xml_print_section_header(WriterContext *wctx) {
    ...
    if (wctx->level == 0) {
        ...
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "<%sffprobe%s>\n",
    ...
    if (section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS) {
        ...
        if (section->flags & SECTION_FLAG_IS_ARRAY) {
            ...
            XML_INDENT();
            PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "<%s ", section->name);
            xml->within_tag = 1;
        }
    }
}
...
static void xml_print_section_footer(WriterContext *wctx) {
    ...
    if (wctx->level == 0) {
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "</%sffprobe>\n", xml->fully_qualified ? "ffprobe:" : "");
...
static void xml_print_str(WriterContext *wctx, const char *key, const char *value) {
    ...
    if (section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS) {
        XML_INDENT();
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "<%s key=\"%s\"",
               section->element_name, xml_escape_str(&buf, key, wctx));
        av_bprint_clear(&buf);
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, " value=\"%s\"/>\n", xml_escape_str(&buf, value, wctx));
    } else {
        if (wctx->nb_item[wctx->level])
            PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, " ");
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s=\"%s\"", key, xml_escape_str(&buf, value, wctx));
    }
...
static void xml_print_int(WriterContext *wctx, const char *key, long long int value) {
    if (wctx->nb_item[wctx->level])
        PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, " ");
    PRINT_LAST(wctx->buffer, PRINT_BUF_SIZE, "%s=\"%lld\"", key, value);
}
...
static void show_frame(WriterContext *w, AVFrame *frame, AVStream *stream,
    ...
    if (av_frame_get_pkt_pos(frame) != -1)
        print_fmt    ("pkt_pos", "%"
                PRId64, av_frame_get_pkt_pos(frame));
    else
...
static int read_interval_packets(WriterContext *w, InputFile *ifile,
    ...
    if (interval->has_start) {
        ...
        if ((ret = avformat_seek_file(fmt_ctx, -1, -INT64_MAX, target, INT64_MAX, 0)) < 0) {
            LOGE( "Could not seek to position %"PRId64": %s\n",
    ...
    av_frame_free(&frame);
    if (ret < 0) {
        LOGE("Could not read packets in interval ");
    ...
    for (i = 0; i < fmt_ctx->nb_streams; i++) {
        ...
        }
        {
            ...
            if ((t = av_dict_get(opts, "", NULL, AV_DICT_IGNORE_SUFFIX))) {
                LOGE("Option %s for input stream %d not found\n",
                       t->key, stream->index);
                return AVERROR_OPTION_NOT_FOUND;
            }
        }
...
{
    if (input_filename) {
//        LOGE("Argument '%s' provided as input filename, but '%s' was already specified.\n",
//               arg, input_filename);
//        exit_program(1);
    }
    if (!strcmp(arg, "-"))
...
static int parse_read_interval(const char *interval_spec,
    ...
    if (!*spec) {
        LOGE( "Invalid empty interval specification\n");
        ret = AVERROR(EINVAL);
        goto end;
    }
    ...
    if (*p) {
        ...
        if (ret < 0) {
            LOGE( "Invalid interval start specification '%s'\n", p);
            goto end;
        }
    } else {
    ...
    if (p && *p) {
        ...
        if (interval->end_is_offset && *p == '#') {
            ...
            if (ret < 0) {
                LOGE( "Invalid interval end/duration specification '%s'\n", p);
                goto end;
            }
            interval->end = us;
        }
...
static int parse_read_intervals(const char *intervals_spec) {
    ...
    for (i = 0; p; i++) {
        ...
        if (ret < 0) {
            LOGE( "Error parsing read interval #%d '%s'\n",
                   i, p);
            goto end;
        }
...
char* probe(int argc, char **argv) {
    ...
    if (!w) {
        LOGE( "Unknown output format with name '%s'\n", w_name);
        ret = AVERROR(EINVAL);
        goto end;
    }
    ...
    if ((ret = writer_open(&wctx, w, w_args,
        ...
        if (!input_filename &&
            ...
            show_usage();
            LOGE( "You have to specify one input file.\n");
...
Java_com_george5613_probe_logic_ProbeLogic_probeVideo(JNIEnv *env, jclass type, jobjectArray commands) {
    jsize argc = (*env)->GetArrayLength(env, commands);
    char **argv = malloc(sizeof(char *) * (argc + 1)); // +1 for fake program name at index 0
    argv[0] = "ffprobe";
    int i;
    for (i = 0; i < argc; ++i) {
        jstring string = (jstring) ((*env)->GetObjectArrayElement(env, commands, i));
    ...
    char *result = probe(argc + 1, argv);
    for (i = 0; i < argc; ++i) free(argv[i + 1]);
    free(argv);
